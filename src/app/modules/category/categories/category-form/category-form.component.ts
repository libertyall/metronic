...
export class CategoryFormComponent implements OnInit, OnDestroy {
	...
	@Output() setSelectedCategory: EventEmitter<Category> = new EventEmitter<Category>(false);
	...
	createForm() {
		this.form = this.fb.group({
			...
			parentCategory: [this.category ? this.category.parentCategory : ''],
			...
			publication: this.fb.group({
				at: [this.category.publication ? this.category.publication.at : new Date()],
				by: [this.category.publication ? this.category.publication.by : ''],
				status: [this.category.publication ? this.category.publication.status : 0]
			}),
	...
	onSubmit(withBack: boolean = false) {
		...
		this.createCategory(editedCategory, withBack);
	}
	...
	prepareCategory(): Category {
		...
		return {
			...
			parentCategory: controls['parentCategory'].value
		};
	}

	createCategory(_category: Category, goToList: boolean = false) {
		console.log(_category, goToList);
